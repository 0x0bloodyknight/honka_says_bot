...
import os, errno, json, gzip, hashlib
# import API keys
...
try:
    ...
    if os.path.exists('renders'):
        log('renders folder created!')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

# load templates once script starts
with open('template.min.json', encoding='utf-8') as temp_json:
    template_sticker = json.load(temp_json)
with open('reverse_template.min.json', encoding='utf-8') as temp_json:
...
async def generateHonka(file_name: str,
                        text: str,
                        size: int = 35,
                        margin: int = 0,
                        top_margin: int = -5,
    ...
    font = FontStyle("Comic Sans MS:style=bold",
                     size,
    ...
    layer = objects.ShapeLayer.load(t_sticker["layers"][2])
    ...
    t_sticker["layers"][2] = layer.to_dict()

    # save to .tgs
    with gzip.open(os.path.join('renders', file_name + '.tgs'),
                   "wb") as final_tgs:
...
async def handler(event):
    ...
    if not text_to_render.endswith('.'):
        await event.answer(results=[
            builder.document(
                utils.resolve_bot_file_id('CAADAgADUAADq1fEC2ZINSTHw8cmAg'),
                type='sticker')
        ],
                           switch_pm='End your query with a dot',
                           switch_pm_param='DOT',
                           cache_time=0,
                           gallery=True)
    else:
        ...
        try:
            ...
            sticker_name = hashlib.md5(
                str(text_to_render).encode('utf-8')).hexdigest()
            ...
            if text_to_render == '':
                await event.answer(results=[
                    builder.document(utils.resolve_bot_file_id(
                        'CAADAgADUAADq1fEC2ZINSTHw8cmAg'),
                                     type='sticker')
                ],
                                   switch_pm='Visit @honka_home for updates',
                                   switch_pm_param='HONK',
                                   cache_time=86400,
                                   gallery=True)
            else:
                ...
                if os.path.exists(
                        os.path.join('renders', sticker_name + '.tgs')):
                    log('Send existing ' + direction + ' stickers for ' +
                        text_to_render + ', hash ' + sticker_name)
                    await event.answer(
                        results=[
                            builder.document(os.path.join(
                                'renders', sticker_name + '-small.tgs'),
                                             type='sticker'),
                            builder.document(os.path.join(
                                'renders', sticker_name + '-medium.tgs'),
                                             type='sticker'),
                            builder.document(os.path.join(
                                'renders', sticker_name + '-large.tgs'),
                                             type='sticker')
                        ],
                        switch_pm='Visit @honka_home for updates',
                        switch_pm_param='HONK',
                    ...
                    log('Generating ' + direction + ' stickers for ' +
                        text_to_render + ', hash ' + sticker_name)
                    await generateHonka(sticker_name + '-medium',
                                        str(text_to_render),
                                        direction=direction)
                    await generateHonka(sticker_name + '-small',
                                        str(text_to_render),
                                        25,
                                        top_margin=-10,
                                        direction=direction)
                    await generateHonka(sticker_name + '-large',
                                        str(text_to_render),
                                        48,
                                        top_margin=1,
                                        direction=direction)
            ...
            log('Got flooded, waiting for ' + identifier.seconds + ' seconds')
            await event.answer([
                builder.article(
                    'ERROR',
                    description='Telegram paused the bot for ' +
                    str(identifier.seconds) + ' seconds. Please wait.',
                    text='Patiently waiting for ' + str(identifier.seconds) +
                    ' seconds and then I HONK!!!')
            ],
                               cache_time=identifier.seconds)
